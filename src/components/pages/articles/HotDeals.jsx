...
const HotDeals = () => {
  ...
  const dealsArticles = articles.filter((item) => item.tags.includes(3));

  const filteredArticles = dealsArticles.filter((dealsArticle) => {
    const query = searchQuery.toLowerCase();
    const title = dealsArticle.title.toLowerCase();
  ...
  return (
    <div className="mb-10">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-semibold lg:mt-4 mb-6">Hot Deals</h3>
        </div>
        <div>
          <SearchField
