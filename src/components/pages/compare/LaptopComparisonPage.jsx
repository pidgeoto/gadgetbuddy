...
const LaptopComparisonPage = ({ modelIdForCard, index, onRemove }) => {
    ...
    <table>
      <tr>
        ...
        <td className="tblDesc">
          <Link href={`/laptop/${item.slug}`} target="_blank">
      ...
      {Object.keys(item).map((key) => {
        if (key.includes("laptop_")) {
          const sanitizedKey = key.replace("laptop_", "");
          return (
            <tr key={key}>
              {index === 0 ? (
                <th>
...
export default LaptopComparisonPage;
